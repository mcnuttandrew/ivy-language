...
import { ListWidget, Template } from "../ivy-lang";
import {
  buildSynthesizer,
  walkTreeAndLookForFields,
  difference,
...
function getDataViews(props: RendererProps): Promise<any> {
  ...
  return view
    ...
    .then(
      () =>
        view.getState({ signals: vega.falsy, data: vega.truthy, recurse: true })
          .data
    );
}
...
function listFind(
  val: string
): (d: { display: string; value: string } | string) => boolean {
...
export function tryToGuessTheTypeForVegaLite(
  ...
  const typeWidget = template.widgets.find(
    (widget) => widget.name === `${payload.field}Type`
  );
  if (!(typeWidget && payload.type === "DataTarget")) {
    return;
  }
  const column = columns.find(
    (col) => col.field === trim(payload.text as string)
  );
...
function inferPossibleDataTargets(spec: any): Set<string> {
  ...
  const foundFields = walkTreeAndLookForFields((key: string) =>
    possibleFields.has(key)
  )(spec);
  const generatedFields = walkTreeAndLookForFields(
    (key: string) => key === "as"
  )(spec);
  return difference(foundFields, generatedFields);
}

function inferRemoveDataSuggestions(
  code: string,
  parsedCode: any
): Suggestion[] {
  const suggestions = [];
...
const VEGA_LITE_CONFIG: LanguageExtension = {
  ...
  suggestion: buildSynthesizer(
    inferPossibleDataTargets,
    inferRemoveDataSuggestions
  ),
  language: "vega-lite",
