...
import { RendererProps, Suggestion } from "../types";
...
function VegaRenderer(props: RendererProps): JSX.Element {
  ...
  const finalSpec = JSON.parse(JSON.stringify(spec));
  // this stratagey only supports one data set
  (Array.isArray(finalSpec.data) ? finalSpec.data : []).forEach(
    (row: any, idx: number) => {
      if (row.values === "myData") {
        finalSpec.data[idx].values = data;
      }
...
function inferRemoveDataSuggestions(
  ...
  const suggestions = [];
  if (
    Array.isArray(parsedCode.data) &&
    parsedCode.data.find((d: any) => d.values)
  ) {
