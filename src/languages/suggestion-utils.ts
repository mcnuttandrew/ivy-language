import { setTemplateValues, GenWidget } from "../ivy-lang";
import { Suggestion, ColumnHeader } from "../types";
import { DataTargetFactory } from "../templates";
import { trim, generateFullTemplateMap } from "../utils";
...
function inferFieldTransformationSuggestions(
  ...
  const widgetNames = widgets.reduce(
    (acc, row) => acc.add(row.name).add(`[${row.name}]`),
    new Set()
  );
  ...
  const dropTargets = widgets
    .filter((widget) => widget.type === "DataTarget")
  ...
  const suggestions = likelyFields.reduce((acc: Suggestion[], from) => {
    ...
    const containsFromNameAlready = !widgets.every(
      (widget) => widget.name !== from
    );
    const suggestedNewWidgetName = !containsFromNameAlready
      ? from
      : `Var${widgets.length + 1}`;
    acc.push({
      from: `"${from}"`,
      ...
      sideEffect: () =>
        DataTargetFactory(
          containsFromNameAlready ? widgets.length + 1 : suggestedNewWidgetName
        ),
    });
...
export function buildSynthesizer(
  ...
  return function synthesizeSuggestions(
    ...
    const parsedCode = safeParse(
      setTemplateValues(code, simulatedCompleteTemplate)
    );
    // if we can't, bail
    if (!parsedCode) {
      return [];
    ...
    const addSuggestion = (suggestion: Suggestion): any =>
      suggestions.push(suggestion);
    inferFieldTransformationSuggestions(
      code,
      parsedCode,
      widgets,
      inferenceFunction,
