...
const SCATTERPLOT: Template = {
  widgets: [
    ...
    },
    {
      ...
      type: "List",
      config: {
        allowedValues: ['"quantitative"', '"ordinal"'].map((x) => ({
          display: x,
          value: x,
        })),
    ...
    },
    {
      ...
      type: "List",
      config: {
        allowedValues: ['"steelblue"', '"blue"', '"red"'].map((x) => ({
          display: x,
          value: x,
        })),
  ...
  code: stringify({
    ...
    mark: {
      type: "point",
      color: { $if: "!parameters.Color", $then: "[Single Color]", $else: null },
    },
    encoding: {
      ...
      color: {
        $if: "parameters.Color",
        $then: { field: "[Color]", type: "[colorType]" },
        $else: null,
      },
    },
  }),
