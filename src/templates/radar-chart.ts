      ...
      marks: [
        {
          ...
          encode: {
            enter: {
              ...
              strokeOpacity: {
                value: {
                  $if: 'parameters.showLines.includes("true")',
                  true: 1,
                  false: 0,
                },
              },
    ...
    // this section is the axes
    {
      ...
      encode: {
        enter: {
          ...
          x2: {
            signal:
              "radius * cos(scale('angular', datum.key) + ([chartAngle] / 360) * PI) + radius",
          },
          y2: {
            signal:
              "radius * sin(scale('angular', datum.key) + ([chartAngle] / 360) * PI) + radius",
          },
    ...
    },
    {
      ...
      encode: {
        enter: {
          x: {
            signal:
              "(radius + 5) * cos(scale('angular', datum.key) + ([chartAngle] / 360) * PI) + radius",
          },
          y: {
            signal:
              "(radius + 5) * sin(scale('angular', datum.key) + ([chartAngle] / 360) * PI) + radius",
          },
          text: { field: "key", format: "%2f" },
          align: [
            {
              test:
                "abs(scale('angular', datum.key) + ([chartAngle] / 360) * PI) > PI / 2",
              value: "right",
            },
            { value: "left" },
          ],
...
const toDisplay = (x: string): { display: string; value: string } => ({
  display: x,
  value: `"${x}"`,
});
  ...
  widgets: [
    ...
    ...cols.reduce((acc: any[], idx) => {
      ...
      acc.push({
        ...
        type: "List",
        config: {
          allowedValues: ["mean", "sum", "median", "min", "count", "max"].map(
            toDisplay
          ),
          defaultValue: '"mean"',
        },
