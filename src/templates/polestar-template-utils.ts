...
(justCountAgg[0] as any).value = undefined;
const spatialAggs = [
  ...justCountAgg,
  ...toList(["min", "max", "sum", "mean", "median", "mode", "distinct"]),
];

export const used = (x: string): ConditionQuery => `Boolean(${x})`;
export const unused = (x: string): ConditionQuery => `!${x}`;
export const notCount = (key: string): ConditionQuery =>
  `!${key}.includes('COUNT')`;
 ...
 */
export const simpleCondition = (key: string): Condition => ({
  queryResult: "hide",
  query: unused(key),
});
...
export const makeMultiTarget = ({
  dim,
  conditions,
}: MakeMultiTargetType): Widget<MultiDataTargetWidget> =>
  ({
    name: dim,
...
export const simpleList = ({
  ...
  return {
    ...
    config: {
      allowedValues: firstDisplayValue
        ? (list as displayType[])
...
export const makeAgg = (key: string): Widget<ListWidget> =>
  simpleList({
    ...
    conditions: [
      {
        queryResult: "show",
        query: `${used(key)} && ${notCount(
          key
        )} && ${key}Type.includes('quantitative')`,
      },
    ],
  });

export const makeTypeSelect = (
  key: string,
  defaultVal: string
): Widget<ListWidget> =>
  simpleList({
    ...
    conditions: [
      simpleCondition(key),
      { queryResult: "show", query: `${used(key)} && ${notCount(key)}` },
    ],
  });
