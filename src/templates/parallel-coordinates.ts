...
const ParallelCoordinates: Template = {
  ...
  templateDescription:
    "Visualize the relationships and trends across a set of quantitative varables",
  templateAuthor: "Ivy Authors",
  templateLanguage: "vega",
  disallowFanOut: true,
  widgets: [
    { type: "Section", name: "warning header" } as any,
    {
      type: "Text",
      name: "repeat warning",
          ...
          query: `${cols
            .map((d, idx) =>
              cols
                ...
                .map(
                  (x) =>
                    `(parameters.Col${d} && (parameters.Col${x} === parameters.Col${d}))`
                )
                .join(" || ")
            )
            .join(" || ")}`,
    ...
    },
    {
      ...
      type: "List",
      config: {
        allowedValues: toList(namedColors),
