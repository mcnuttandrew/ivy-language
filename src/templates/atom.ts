...
function generateLevel(idx: number): { widgets: GenWidget[]; layout: any } {
  ...
  const widgets: GenWidget[] = [
    ...
    simpleList({
      ...
      list: [
        ...ASPECT_RATIOS.map((d) => ({ display: d, value: `"${d}"` })),
        { display: "null", value: "null" },
      ],
      conditions: [
        !NEVER_HIDE && {
    ...
    }),
    {
      ...
      conditions: [
        { queryResult: "hide", query: `parameters.Key${idx}Type !== '"bin"'` },
      ],
  ...
  return {
    ...
    layout: cond(!NEVER_HIDE ? `parameters.Key${idx - 1}` : "true", {
      subgroup: {
        ...
        numBin: cond(
          `parameters.Key${idx}Type === '"bin"'`,
          `[Key${idx}numBins]`
        ),
      },
      aspect_ratio: cond(
        `parameters.Key${idx}AspectRatio`,
        `[Key${idx}AspectRatio]`
      ),
    }),
  };
...
const ATOM: Template = {
  ...
  widgets: [
    ...
    ...configurations.reduce((acc, d) => acc.concat(d.widgets as any), []),
  ],
};
export default ATOM;
