...
import {
  VEGA_CATEGORICAL_COLOR_SCHEMES,
  VEGA_CONT_COLOR_SCHEMES,
} from "./vega-common";
...
const paramsInclude = (key: string): string =>
  `Object.values(parameters).includes('\\"${key}\\"')`;
const eitherMeta = `${paramsInclude(META_COL_ROW)} || ${paramsInclude(
  META_COL_COL
)}`;
const USING_META_COLS_CONDITION: Condition = {
  queryResult: "show",
  query: eitherMeta,
};
...
const injectCondition = (
  ...
  ...widget,
  conditions: [...(widget.conditions as any), condition],
});
...
const encoding = {
  ...["X", "Y"].reduce((acc: JsonMap, key) => {
    const output = {
      ...
      scale: {
        $if: notCount(key),
        true: { zero: zeroConditional(key), type: typeCond(key) },
      },
...
const Polestar: Template = {
  ...
  customCards: [
    {
      name: "COUNT",
      description:
        "use this card to treat channels as simple count aggregations",
    },
    {
      name: META_COL_ROW,
      description: "place this card onto fields to facet across by data column",
    },
    {
      name: META_COL_COL,
      description: "place this card onto fields to facet across by data column",
  ...
  widgets: [
    ...
    simpleList({
      name: "markType",
      list: toList(MARK_TYPES),
      defaultVal: toQuote("point"),
    }),
      ...
      const widgets = [
        ...
        simpleSwitch({
          ...
          conditions: [
            ...
            {
              queryResult: "show",
              query: `${notCount(key)} && ${typeIsQuant}`,
            },
          ],
        }),
        makeAgg(key),
      ...
      if (key === "Color") {
        ...
        widgets.push({
          ...
          conditions: [
            ...
            {
              query:
                '!ColorType.includes("nominal") || Color.includes("COUNT")',
              queryResult: "hide",
            },
          ],
    ...
    injectCondition(makeText("Repeat Small Multiply", []), {
      query: eitherMeta,
      queryResult: "hide",
    }),
    ...["Row", "Column"].reduce(
      (acc: GenWidget[], key: string) =>
        acc.concat([
          injectCondition(makeDataTarget(key), {
            query: eitherMeta,
            queryResult: "hide",
          }),
        ]),
    ...
    }),
    {
      ...
      conditions: [
        { query: '!showHeight.includes("true")', queryResult: "hide" },
      ],
    },
    simpleSwitch({
      name: `showWidth`,
      displayName: "Specify Width",
      defaultsToActive: false,
    }),
    {
      name: "width",
      ...
      config: { minVal: 20, maxVal: 800, step: 10, defaultValue: 100 },
      conditions: [
        { query: '!showWidth.includes("true")', queryResult: "hide" },
      ],
    },
  ],
};
